...
## Properties include the current weather state, weather intensity, and transition speed.
...
## The timer is used internally to trigger changes in the weather state.
...
func adjust_intensity(new_intensity: int) -> void:
    weather_intensity = clamp(new_intensity, 0, 100)
    print("Weather intensity adjusted to {0}.".format([weather_intensity]))

## Function to get the current weather state as a string.
## Returns a human-readable string representation of the current weather state.
## Returns:
##   The current weather state as a string (e.g., "Clear", "Rainy", "Stormy").
func get_weather_state_string() -> String:
...
## Function to print the current weather status.
## Outputs a summary of the current weather state and intensity to the console.
func print_weather_status() -> void:
    print("Current Weather: {0}, Intensity: {1}".format(
        [get_weather_state_string(), weather_intensity]))
